...
export default function Picker ({ openPicker, setOpenPicker, pickerType, setPickerType, yearRange, hr24, dateLimits }) {
  ...
  const [selectedPicker, setSelectedPicker] = useState(
    <DatePicker pickerType={ pickerType } setPickerType={ setPickerType } yearRange={ yearRange } dateLimits={ dateLimits } />
  );
  ...
  useEffect(() => {
    ...
    if (pickerType === 'calendar' || pickerType === 'year') {
      setSelectedPicker(<DatePicker pickerType={ pickerType } setPickerType={ setPickerType } yearRange={ yearRange } dateLimits={ dateLimits } />);
    }
    if (pickerType === 'hours' || pickerType === 'mins') {
      setSelectedPicker(<TimePicker pickerType={ pickerType } setPickerType={ setPickerType } hr24={ hr24 } dateLimits={ dateLimits } />);
    }
  }, [pickerType]);
        ...
        <div ref={ pickerBackground } className={ `${styles[pickerStyle]}` }>
          <div className={ styles.pickerContainer }>
            ...
            <div className={ styles.selectedUnderlineContainer }>
              <div
                className={ `${styles.selectedUnderline} ${pickerType === 'hours' || pickerType === 'mins' ? styles.underlineRight : styles.underlineLeft
                  }` }></div>
            </div>
